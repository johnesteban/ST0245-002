...
class GraphAm:
    ...
    def hayCaminoBFS(self,origen,destino):
        visitados=[False]*self.size
        cola=deque()
        cola.append(origen)
        while(len(cola)!=0):
            soyElQueEstabaDePrimero=cola.popleft()
            visitados[soyElQueEstabaDePrimero]=True
            if(soyElQueEstabaDePrimero==destino):
                return True
            for vertice in self.getSuccessors(soyElQueEstabaDePrimero):
                if visitados[vertice]==False:
                    cola.append(vertice)
        return False
