...
import pandas as pd
import InterpolacionBilinear as ib
...
class estructura():
    ...
    def compresion(self,carpeta,arreglo):
        for i in range(len(arreglo)):  
            ImgCom=ib.interpolacion(arreglo[i]["arreglo"],0.5).astype(np.uint8) #Hago la interpolacion bilinear de cada archivo
    ...
    def descompresion(self,carpetaOrigen,carpetaDestino):
        for i in os.listdir(carpetaOrigen):
            ...
            arreglo=ib.interpolacion(np.array(im),2).astype(np.uint8) #Se hace la interpolacion ampliando la imagen al tamaño original
